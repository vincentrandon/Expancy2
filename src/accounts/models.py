...
class Supplement(models.Model):
    ...
    supplement_manutention = models.DecimalField('Supplément manutention', max_digits=7, decimal_places=3, blank=True, null=True)
    supplement_gt = models.DecimalField('Supplément GT', max_digits=7, decimal_places=3, blank=True, null=True)
    supplement_carburant_routier = models.DecimalField('Supplément carburant routier', max_digits=7, decimal_places=3, blank=True, null=True)
    supplement_frais_de_gestion = models.DecimalField('Supplément frais de gestion', max_digits=7, decimal_places=3, blank=True, null=True)
    supplement_facture_papier = models.DecimalField('Supplément facture papier', max_digits=7, decimal_places=3, blank=True, null=True)
    supplement_surete_colis = models.DecimalField('Supplément facture colis', max_digits=7, decimal_places=3, blank=True, null=True)
    supplement_zone_internationale_eloignee = models.DecimalField('Supplément zone internationale éloignée', max_digits=7, decimal_places=3, blank=True, null=True)
    supplement_surcharge_carburant_routier = models.DecimalField('Supplément surcharge carburant routier', max_digits=7, decimal_places=3, blank=True, null=True)
    supplement_covid = models.DecimalField('Supplément COVID', max_digits=7, decimal_places=3, blank=True, null=True)
    supplement_taxe_carbone = models.DecimalField('Supplément taxe carbone', max_digits=7, decimal_places=3, blank=True, null=True)
    slug = models.SlugField(max_length=255, unique=True, blank=True)
    files = models.ManyToManyField(CheckFile)
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            if self.brand:
                self.slug = slugify(str("tarifs-") + str(self.company) + "-" + str(self.brand) + "-" + str(self.transporter))
            else:
